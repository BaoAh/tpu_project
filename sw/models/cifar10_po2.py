...
# Layer
   ...
   #      model.add(tf.keras.layers.AveragePooling2D(pool_size=1, strides=1))
    ...
    model.add(tf.keras.layers.Flatten())
    model.add(QDense(1024, activation='relu',use_bias=False,
            kernel_quantizer="quantized_po2(8,quadratic_approximation=True)",name = 'fc1'))
    model.add(tf.keras.layers.Dropout(0.5))
    model.add(QDense(10, activation='softmax',use_bias=False,
            kernel_quantizer="quantized_po2(8,quadratic_approximation=True)",name = 'output_fc2'))
